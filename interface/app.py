...
def fetch_scores_from_db(conn):
    ...
    - Последние 500 скоров для гистограммы.
    - Последние 500 скоров со временем для линейного графика.
    ...
    try:
        ...
        # 2. Запрос последних 500 скоров со временем для графиков
        # Важно: выбираем 500 последних записей (DESC)
    ...
    if st.button("Посмотреть результаты"):
        ...
        if conn:
            try:
                ...
                st.subheader("📈 Гистограмма распределения скоров (последние 500 транзакций):")
                ...
                st.subheader("📊 График скоров последних 500 транзакций (по времени):")
                if not df_scores_plot.empty:
                    ...
                    ax_line.set_title('Скор транзакций по времени (последние 500)')
