...
## 🛠️ Использование
### 1. Загрузка данных:
 - Загрузите CSV через интерфейс Streamlit. `http://localhost:8501`

   
 - Для тестирования работы проекта используется файл формата `test.csv` из соревнования [Teta ML-1 2025](https://www.kaggle.com/competitions/teta-ml-1-2025).
...
- Нажмите  **"Отправить транзакции на обработку"** 
   ...
   - При отправке большого файла данных,  можно прервать отправку в любой момент, перейдя во вкладку `Результаты Скоринга` и посмотрев резултаты. 
- Перейдите во вкладку `Результаты Скоринга` и нажмите `Посмотреть результаты`.
