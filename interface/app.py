...
# --- Определяем список колонок CSV вручную ---
# Этот список должен ТОЧНО соответствовать порядку колонок в вашем CSV-файле
# и включать все колонки, которые нужны для препроцессинга и скоринга.
# Если ваш CSV имеет другой порядок или другие колонки, обновите этот список.
CSV_COLUMNS = [
    "transaction_time", "vendor", "category", "amount", "first_name", "last_name", "gender",
    "address", "city", "us_state", "zip", "lat", "lon", "population_city", "jobs",
...
# --- Основной раздел: Отправка и Результаты ---
...
if uploaded_file:
    ...
    if st.button("Отправить транзакции на обработку"):
        ...
        if producer:
            try:
                # !!! КЛЮЧЕВОЕ ИЗМЕНЕНИЕ ЗДЕСЬ: чтение CSV без заголовков и присвоение имён !!!
                # Используем header=None, names=CSV_COLUMNS для корректного чтения CSV без заголовков
                df = pd.read_csv(uploaded_file, header=None, names=CSV_COLUMNS)
                st.write("Первые 3 строки из файла (с присвоенными колонками):", df.head(3))
                ...
                for idx, row in df.iterrows():
                    ...
                    message = {
                        ...
                        "data": row.to_dict() # Теперь row.to_dict() будет содержать правильные имена колонок
                    }
                ...
                st.exception(e) # Показываем полный стек ошибок для отладки
        else:
            st.warning("Продюсер Kafka не инициализирован. Проверьте подключение к Kafka.")

# Раздел "Результаты Скоринга"
