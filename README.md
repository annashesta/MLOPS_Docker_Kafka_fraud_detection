...
Для запуска сервиса вам понадобится скачать предобученную модель, а так же файлы с транзакциями для теста:
...
├── kafka-setup.sh                   # Скрипт для инициализации Kafka (создание топиков)
├── test_transactions.csv            # Пример файла с тестовыми транзакциями
└── README.md                        # Этот файл
```
...
cd <ИМЯ_КАТАЛОГА_ВАШЕГО_РЕПОЗИТОРИЯ>
```
...
Дождитесь, пока все контейнеры будут запущены и станут `Healthy` или `Started`. Вы можете проверить их статус, выполнив:
...
2.  В этом интерфейсе вы найдете раздел для отправки тестовых транзакций.
3.  Нажмите кнопку **"Отправить транзакцию в Kafka"** (или аналогичную, в зависимости от вашей реализации).
      * Вы можете отправлять несколько транзакций. Каждая транзакция из `test.csv` будет отправлена как отдельное сообщение в топик `transactions`.
	-  Для теста представлен маленький (23 транзакции) и большой( ) файлы, отправку большого потока данных можно прервать, перейдя в результаты и нажав "посмотреть результаты".
